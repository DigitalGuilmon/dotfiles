...
audioOptions =
    ...
    , ("6. Salida: Altavoces",      spawn "id=$(pw-dump 2>/dev/null | jq -r '.[] | select(.info.props[\"node.description\"]? // \"\" | test(\"Speaker|Altavoz\"; \"i\")) | .id' 2>/dev/null | head -1) && [ -n \"$id\" ] && wpctl set-default \"$id\" && notify-send '🔈 Audio' 'Altavoces seleccionados' || notify-send '⚠️ Audio' 'No se encontraron altavoces'")
    , ("7. Salida: Auriculares",    spawn "id=$(pw-dump 2>/dev/null | jq -r '.[] | select(.info.props[\"node.description\"]? // \"\" | test(\"Headphone|Auricular\"; \"i\")) | .id' 2>/dev/null | head -1) && [ -n \"$id\" ] && wpctl set-default \"$id\" && notify-send '🎧 Audio' 'Auriculares seleccionados' || notify-send '⚠️ Audio' 'No se encontraron auriculares'")
    ]
