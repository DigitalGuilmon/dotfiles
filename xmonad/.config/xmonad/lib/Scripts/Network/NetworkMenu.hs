...
networkOptions =
    ...
    , ("5. VPN: Conectar",             spawn $ "VPN=$(nmcli -t -f NAME,TYPE connection show | grep vpn | cut -d: -f1 | rofi -dmenu -p 'VPN:' -theme " ++ myTheme ++ ") && [ -n \"$VPN\" ] && nmcli connection up id \"$VPN\" && notify-send '🔒 VPN' 'VPN conectada'")
    , ("6. VPN: Desconectar",          spawn $ "VPN=$(nmcli -t -f NAME,TYPE connection show --active | grep vpn | cut -d: -f1 | rofi -dmenu -p 'Desconectar VPN:' -theme " ++ myTheme ++ ") && [ -n \"$VPN\" ] && nmcli connection down id \"$VPN\" && notify-send '🔓 VPN' 'VPN desconectada'")
