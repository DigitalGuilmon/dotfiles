...
def ranked_history_labels(history: dict[str, dict[str, int]]) -> list[str]:
    ranked = []
    ...
    ranked.sort()
    return [label for _, _, _, label in ranked]
...
def menu_options(options: list[str], history: dict[str, dict[str, int]]) -> list[str]:
    if options:
        return reorder_options(options, history)
    return ranked_history_labels(history)


def should_store_selection(selection: str) -> bool:
    return bool(selection) and len(selection) <= MAX_STORED_LABEL_LENGTH


def prune_history(history: dict[str, dict[str, int]]) -> dict[str, dict[str, int]]:
    keep: dict[str, dict[str, int]] = {}
    for label in ranked_history_labels(history)[:MAX_HISTORY_ENTRIES]:
        keep[label] = history[label]
    return keep
...
def main():
    ...
    history_enabled = bool(args.menu_id) and not args.no_history
    ...
    if history_enabled:
        ...
        ordered_options = menu_options(options, history)
    ...
    if (
        result.returncode == 0
        and history_enabled
        and should_store_selection(selection)
        ...
        save_history(path, prune_history(history))
