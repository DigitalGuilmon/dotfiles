module Shared.Menu.Timer (timerMenu) where

import XMonad
import Shared.Utils (rofiInput, rofiSelect, shellEscape)
...
timerMenu :: X ()
timerMenu = do
    res <- rofiSelect "xmonad-timer-menu" "Timer:" ["-i"] timerOptions
    case res of
